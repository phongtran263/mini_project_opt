...
class TimetableSchedule():
    ...
    def value(self, state, flag = False):
        #heuristics function
        violated_classes = set()
        violations = 0
        d1 = {}
        d2 = {}
        ...
        for i in d2.values():
            d3 = {}
            for j in i:
                d3[self.g[j]] = d3.get(self.g[j],set())
                d3[self.g[j]].add(j)
            for i,j in d3.items():
                if len(j) > 1:
                    violated_classes = violated_classes.union(j)
            violations += sum([len(i) - 1 for i in d3.values()])
        
        for i,j in d1.items():
            if j > 0:
                violated_classes.add(i[0])
        violated_variables = [(i,j) for i in violated_classes for j in range(self.t[i])]
        if flag:
